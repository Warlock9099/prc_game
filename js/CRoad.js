function CRoad(oRoadDrawingLevel, oElementContainer, iLevel, oRumbleDrawingLevel, oGrassDrawingLevel, oLaneDrawingLevel, oRumbleStrokeLevel){    
    ...
    this.update = function(oPlayerPos){
        ...
        // console.log("playerSegment",playerSegment)//1444
        ...
        for(n = 0 ; n < DRAW_DISTANCE ; n++) {
            ...
            Render.mud(oRumbleDrawingLevel, WIDTH_ROTATION_VIEW, NUM_LANES,
                oPoints.x1,
                oPoints.y1,
                oPoints.w1,
                oPoints.x2,
           ...
           var segmentGroupSize = 20
           var groupIndex = Math.floor(segment.index/segmentGroupSize)
           var isWhiteLine = groupIndex%4 ==0
            if (isWhiteLine) {
                    Render.whitelane(oRumbleDrawingLevel, WIDTH_ROTATION_VIEW, NUM_LANES,
                        oPoints.x1,
                        oPoints.y1,  
                        oPoints.w1,
                        oPoints.x2,
                    ...
                    var g_gap = 10
                    var g_index = Math.floor(segment.index/g_gap)
                    var isGreen = g_index%10 ==0
                            ...
                            if(isGreen){
                                ...
                                Render.grass_texture(oGrassDrawingLevel, WIDTH_ROTATION_VIEW, NUM_LANES,
                                    oPoints.x1,
                                    oPoints.y1,  
                                    oPoints.w1,
                                    oPoints.x2,
        ...
        for(n = (DRAW_DISTANCE-1) ; n > 0 ; n--) {
            ...
            */
            ...
            for(var i = 0 ; i < segment.elements.length ; i++) {
                ...
                if(sprite._oData.name == "start_position" || sprite._oData.name == "lstart_position"){
                    sprite.setVisible(bool) 
                }
                if(s_oGame.rumbleCondition(playerSegment)){
                    bool = false
                   }
                if(sprite._oData.name == "coin"){
                    sprite.setVisible(COIN_BOOL)
                } 
