var Render = {
  ...
  grass_texture: function(ctx, width, lanes, x1, y1, w1, x2, y2, w2, alpha) {
    var image = new Image();
    image.src = './sprites/grass1_png.png';

    image.onload = function(){
        ctx.graphics.beginBitmapFill(image, 'repeat').setStrokeStyle(0.2).beginStroke("#2D992D").drawRect(START_X_ROTATION_VIEW, y2, width, y1 - y2);
    }
},

  mud :function(ctx, width, lanes, x1, y1, w1, x2, y2, w2, color){
      ...
      Render.simplePolygon(ctx, x1-w1-r1*6, y1, x1-w1*8, y1, x2-w2*1.2, y2, x2-w2-r2, y2, "#2D992D");
      Render.simplePolygon(ctx, x1+w1+r1*6, y1, x1+w1*8, y1, x2+w2*1.2, y2, x2+w2+r2, y2, "#2D992D");
    
},
  
  grass:function(ctx, width, lanes, x1, y1, w1, x2, y2, w2, color){
    ctx.graphics.beginStroke("#2D992D").beginFill("#2D992D").drawRect(START_X_ROTATION_VIEW, y2, width, y1 - y2);
    // ctx.graphics.drawRect(START_X_ROTATION_VIEW, y2, width, y1 - y2);

  },

    coin: function(ctx, width, lanes, x1, y1, w1, x2, y2, w2, color) {
      //ctx.graphics.beginFill(color.grass);
      ctx.graphics.drawCircle(START_X_ROTATION_VIEW, y2, width, y1 - y2);
  },

  whitelane:function(ctx, width, lanes, x1, y1, w1, x2, y2, w2, color){
    var r1 = Render.rumbleWidth(w1, lanes),
        r2 = Render.rumbleWidth(w2, lanes),
        l1 = Render.laneMarkerWidth(w1, lanes),
        l2 = Render.laneMarkerWidth(w2, lanes);
    Render.simplePolygon(ctx, x1 - l1, y1, x1+l1/2, y1, x2+l2/2, y2, x2-l2, y2,color);
  },
